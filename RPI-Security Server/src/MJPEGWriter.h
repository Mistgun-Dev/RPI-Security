/*
*   Fichier permettant l'envoi des frames de caméra sur un serveur web, permettnat ainsi la visualisation du flux vidéo de la caméra depuis un naviguateur web, 
*   ainsi que la gestion des connexion multiples des clients
*
*   Source : https://github.com/JPery/MJPEGWriter
*/
...
class MJPEGWriter
{
    ...
    int _write(int sockk, char *s, int len)
    {
        if (len < 1) { len = strlen(s); }
        {
        	try
        	{
        		int retval = ::send(sockk, s, len, 0x4000);
        		return retval;
        	}
        	catch (int e)
        	{
